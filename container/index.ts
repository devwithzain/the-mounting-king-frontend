export { default as HomeAbout } from './home/about';
export { default as HomeHero } from './home/hero';
export { default as HomeServices } from './home/services';

export { default as AboutHero } from './about/hero';
export { default as AboutServices } from './about/services';

export { default as Cart } from './cart/cart';

export { default as ThankYou } from './thankyou/thank-you';

export { default as Form } from './book-now/form';
export { default as BookService } from './book-now/book-service';
export { default as RequestServices } from './book-now/request-services';

export { default as ServiceHero } from './services/hero';
export { default as ServicesServices } from './services/services';

export { default as ContactHero } from './contact/hero';
export { default as ContactForm } from './contact/form';

export { default as ServiceDetailHero } from './serviceDetail/hero';
export { default as ServiceDetailInfo } from './serviceDetail/info';